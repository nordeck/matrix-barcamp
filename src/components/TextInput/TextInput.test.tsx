...
describe('<TextInput>', () => {
  ...
  it('should not accept more than 10 characters', async () => {
    ...
    await userEvent.click(input);
    await userEvent.clear(input);
    await userEvent.type(input, 'Welcome here!');

    expect(input).toHaveValue('Welcome here!');

    expect(onChange).lastCalledWith('Welcome here!');
    expect(onBlur).not.toBeCalled();

    await userEvent.tab();

    expect(onChange).toHaveBeenLastCalledWith('Welcome here!');
    expect(onBlur).not.toBeCalled();
